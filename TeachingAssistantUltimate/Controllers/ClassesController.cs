...
namespace TeachingAssistantUltimate.Controllers
{
    public class ClassesController : Controller
    {
        ...
        [HttpGet]
        public async Task<IEnumerable> List(string id) => await new ApplicationDbContext(dco).Classes.Select(x => new { x.Concurrency, x.Students.Count, x.Name, x.ClassesID, x.IndexPrefix }).ToListAsync();
        ...
        public async Task<IActionResult> Create([FromBody]Classes classes)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    if (await db.Classes.AnyAsync(x => x.Name == classes.Name))
